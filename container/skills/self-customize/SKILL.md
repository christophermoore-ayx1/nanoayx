---
name: self-customize
description: Customize your own agent — add capabilities, install packages, add MCP servers, edit code or CODEX.local.md. Use when the user asks you to add a feature, install a tool, or modify how you work. For non-trivial code changes, delegate to a builder agent via create_agent.
---
...
- **`CODEX.local.md` or files in your workspace** → Edit directly, no approval needed. Your workspace (`/workspace/agent/`) is persisted on the host. (Note: the composed `CODEX.md` itself is read-only and regenerated every spawn — write to `CODEX.local.md` instead.)
