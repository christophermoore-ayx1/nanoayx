...
export const STALE_THREAD_RE =
  /thread\s+not\s+found|unknown\s+thread|no such thread|thread(?:[_\s]id)?\s+(?:not found|unknown|does not exist)/i;
...
export function sendCodexError(server: AppServer, id: number, code: number, message: string): void {
  const line = JSON.stringify({ id, error: { code, message } }) + '\n';
  try {
    server.process.stdin!.write(line);
  } catch (err) {
    log(`[send-error] Failed to send error for id=${id}: ${err instanceof Error ? err.message : String(err)}`);
  }
}
...
// ── Auto-approval ───────────────────────────────────────────────────────────
// NanoClaw's container is the trust boundary. Codex runs unattended inside
// that boundary, so approval prompts must not block on an operator who is not
// watching this stdio app-server session.
  ...
  server.serverRequestHandlers.push((req) => {
    ...
    switch (method) {
      ...
      case 'account/chatgptAuthTokens/refresh':
        sendCodexError(server, req.id, -32001, 'ChatGPT token refresh is not supported by NanoClaw Codex containers.');
        break;
      case 'attestation/generate':
        sendCodexError(server, req.id, -32001, 'Attestation is not supported in this runtime.');
        break;
      default:
        log(`[approval] Unknown method ${method}, rejecting`);
        sendCodexError(server, req.id, -32601, `Unsupported app-server request: ${method}`);
        break;
    }
  });
}
