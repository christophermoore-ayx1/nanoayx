...
// base, module fragments, and per-group memory are available to the app-server
// provider in a single baseInstructions payload.
...
function readAgentAndGlobalCodexMd(): string | undefined {
  // Prefer Codex-specific prompt files, while keeping Claude-named files as
  // a migration fallback for older group folders.
  const groupDir = '/workspace/agent';

  const groupPath = firstExisting([
    `${groupDir}/CODEX.md`,
    `${groupDir}/CLAUDE.md`,
  ]);

  const localPath = firstExisting([
    `${groupDir}/CODEX.local.md`,
    `${groupDir}/CLAUDE.local.md`,
  ]);
...
function composeBaseInstructions(promptAddendum: string | undefined): string | undefined {
  const codexMd = readAgentAndGlobalCodexMd();
  const pieces = [codexMd, promptAddendum].filter((s): s is string => Boolean(s));
  ...
  const handler = (n: JsonRpcNotification): void => {
    ...
    switch (method) {
      ...
      case 'error': {
        ...
        const willRetry = (params as { willRetry?: boolean }).willRetry === true;
        if (e?.message) buffer.push({ type: 'progress', message: `codex error: ${e.message}` });
        if (!willRetry) {
          turnState.error = new Error(e?.message || 'Codex app-server error');
          turnDone = true;
        }
        break;
      }
      case 'thread/status/changed': {
        const status = params.status as { type?: string } | string | undefined;
        const statusText = typeof status === 'string' ? status : status?.type;
        if (statusText) buffer.push({ type: 'progress', message: `status: ${statusText}` });
        break;
      }
      default:
