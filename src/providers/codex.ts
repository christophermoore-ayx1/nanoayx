...
registerProviderContainerConfig('codex', (ctx) => {
  ...
  let authCopyPath: string | null = null;
  const hostHome = ctx.hostEnv.HOME;
  if (hostHome) {
    const hostAuth = path.join(hostHome, '.codex', 'auth.json');
    if (fs.existsSync(hostAuth)) {
      authCopyPath = path.join(codexDir, 'auth.json');
      fs.copyFileSync(hostAuth, authCopyPath);
      fs.chmodSync(authCopyPath, 0o600);
    }
  }
  ...
  if (authCopyPath) {
    mounts.push({ hostPath: authCopyPath, containerPath: '/home/node/.codex/auth.json', readonly: true });
  }
